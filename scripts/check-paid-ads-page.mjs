...
const structuralFailures = [
  ...
  !/visibilitychange/.test(paidAdsPageSource) && "Live previews must respond to document visibility changes.",
  !/prefers-reduced-motion:\s*reduce/.test(paidAdsPageSource) && "Live previews must honor reduced-motion preferences.",
  !/addEventListener\(\s*["']change["']/.test(paidAdsPageSource) && "Reduced-motion handling must respond to live preference changes.",
  !/showPoster:\s*motionPreference\s*!==\s*["']no-preference["']\s*\|\|\s*videoFailed/.test(paidAdsPageSource) && "Reduced-motion users must receive the static engine poster.",
